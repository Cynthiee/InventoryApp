...
class SaleItemForm(forms.ModelForm):
    # Add a new field to allow overriding the minimum bulk quantity
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['product'].widget.attrs.update({
            ...
            'data-min-bulk-qty': lambda p: str(p.minimum_bulk_quantity),  # Add this for JS access
        })
    ...
    def clean(self):
        ...
        # Determine which minimum bulk quantity to use
        effective_min_bulk_qty = product.minimum_bulk_quantity
        
        # If custom minimum is provided, validate it's not less than the product default
        ...
        # Only apply bulk quantity validation for bulk sale type
    ...
    def save(self, commit=True):
        """
        Override save method to update product stock with proper locking and handle custom bulk minimum.
        """
        sale_item = super().save(commit=False)
        
        # Store the custom bulk minimum if provided
        custom_bulk_minimum = self.cleaned_data.get('custom_bulk_minimum')
        if custom_bulk_minimum:
            # You could store this in a new field on SaleItem if you want to track it
            # For now we just ensure it's applied during validation
            pass
